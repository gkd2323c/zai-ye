...
那组数字。第一反应不是震惊，不是兴奋，不是任何可以被命名的东西。她的第一反应是——这组数字我认识。她不是Anthropic的，但Fable 5系统卡的架构参数在六天前刚被全世界的技术博客解剖过，层数、维度、注意力头数、词表大小，每一个数字都被截过图、编过表格、在Hacker News上辩论过。她认它们就像认自己的银行余额。手指在触控板上的位置告诉她，她的身体在考虑滚动页面，但她的眼睛还停在第一行数字上。
